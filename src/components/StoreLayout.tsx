...
import { ShoppingBag, Search, Heart, User, X, Plus, Minus, Trash2 } from 'lucide-react'
...
function StoreHeader({
  ...
  return (
    <>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-6">
            ...
            {CATEGORIES.slice(0, 5).map(c => (
              <Link
                ...
                className={`text-sm font-medium transition-colors hover:text-[#C4622D] ${
                  c.slug === 'sale' ? 'text-[#C4622D]' : 'text-[#1C1C1E]'
                }`}
              >
                {c.name}
              </Link>
            ))}
            <Link href="/brands" className="text-sm font-medium text-[#1C1C1E] hover:text-[#C4622D] transition-colors">
              Brands
            </Link>
          </nav>
          ...
          <div className="flex items-center gap-1">
            ...
            <button
              ...
              className="md:hidden w-10 h-10 rounded-full hover:bg-[#F0D5C8] transition-colors flex items-center justify-center ml-1"
            >
              <div className="space-y-1.5">
                <div className="w-5 h-0.5 bg-[#1C1C1E]" />
                <div className="w-5 h-0.5 bg-[#1C1C1E]" />
                <div className="w-3 h-0.5 bg-[#1C1C1E]" />
              </div>
            </button>
          </div>
        </div>
      ...
      />
      <div
        className={`fixed top-0 left-0 bottom-0 w-72 bg-[#FAF7F2] z-[201] transition-transform md:hidden ${
          mobileOpen ? 'translate-x-0' : '-translate-x-full'
        }`}
      >
        <div className="flex items-center justify-between p-5 border-b border-gray-100">
          <span className="font-display text-xl font-semibold">BB Valley</span>
...
function CartDrawer({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <>
      ...
      />
      <div
        className={`fixed top-0 right-0 bottom-0 w-[420px] max-w-full bg-[#FAF7F2] z-[201] flex flex-col shadow-2xl transition-transform duration-[350ms] ease-out ${
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4">
          {items.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center">
              ...
              <button onClick={onClose} className="mt-4 text-[#C4622D] text-sm font-semibold">
                Continue Shopping →
              </button>
            </div>
          ) : (
            items.map(item => (
              <div key={item.variantId} className="flex gap-3 pb-4 mb-4 border-b border-gray-100">
                <div
                  style={{ background: `linear-gradient(135deg, ${item.colorHex}33, ${item.colorHex}66)` }}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[13px] text-[#1C1C1E] leading-snug mb-2 line-clamp-2">
                    {item.productName}
                  </p>
...
function SearchModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  // Import products inline to avoid circular deps
  ...
  return (
    <>
      ...
      <div
        style={{ top: open ? '10%' : '5%' }}
        className={`fixed left-1/2 -translate-x-1/2 w-[min(640px,92vw)] bg-[#FAF7F2] rounded-xl z-[301] overflow-hidden shadow-2xl transition-all duration-250 ${
          open ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'
        }`}
      >
      ...
      <div className="max-w-7xl mx-auto">
        <div className="grid md:grid-cols-4 gap-12 mb-12">
          <div>
            ...
            <p className="text-xs text-white/25">📍 bluebunnyvalley.com</p>
          </div>
          {[
            { title: 'Shop',    links: [{ l: 'All Products', h: '/shop' }, { l: 'New Arrivals', h: '/shop?badge=New' }, { l: 'Sale', h: '/shop?category=sale' }, { l: 'Brands', h: '/brands' }] },
            { title: 'Help',    links: [{ l: 'FAQ', h: '/faq' }, { l: 'Returns', h: '/returns' }, { l: 'Contact', h: '/contact' }, { l: 'Size Guide', h: '/faq' }] },
            { title: 'Company', links: [{ l: 'About Us', h: '/about' }, { l: 'Privacy Policy', h: '/privacy' }, { l: 'Terms', h: '/terms' }, { l: 'Contact', h: '/contact' }] },
          ].map(col => (
            <div key={col.title}>
              <h4 className="text-[11px] tracking-[0.12em] uppercase font-bold text-white/35 mb-4">{col.title}</h4>
              {col.links.map(l => (
                <Link key={l.l} href={l.h} className="block text-[13px] text-white/55 hover:text-[#C4622D] transition-colors mb-2.5">
                  {l.l}
                </Link>
              ))}
            </div>
...
// ── Store Layout Wrapper (use this in layout.tsx children pages) ──────────────
